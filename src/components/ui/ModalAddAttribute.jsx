...
const ModalAddAttribute = ({addEntityElement}) => {
  ...
  return (
    <div className="fixed bg-black bg-opacity-50 flex flex-col justify-center rounded-md z-50">
        ...
        >Agregar llave primaria</button>
        <button className="text-white text-md font-semibold border-b border-b-slate-300 text-center py-2 px-3 cursor-pointer hover:bg-gray-800"
        onClick={() => addEntityElement(opcionesEntidad.RELACION)}
        >Agregar relacion</button>
        <button className="text-white text-md font-semibold border-b border-b-slate-300 text-center py-2 px-3 cursor-pointer hover:bg-gray-800"
        onClick={() => addEntityElement(opcionesEntidad.FK)}
        >Agregar llave foranea</button>
        <button className="text-white text-md font-semibold py-2 px-3 text-center cursor-pointer hover:bg-gray-800"
        onClick={() => {
            setShowModal(true)
            //addEntityElement(opcionesEntidad.TXT)
        }}
        >Agregar nombre a la entidad</button>
        {showModal && (
